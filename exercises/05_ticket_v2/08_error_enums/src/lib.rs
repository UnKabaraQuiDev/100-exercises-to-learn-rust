...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(n) => n,
        Err(TicketNewError::InvalidDescription) => {
            easy_ticket(title, "Description not provided".into(), status)
        }
        Err(TicketNewError::InvalidTitle { message }) => panic!("{}", message),
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::InvalidTitle {
                message: "Title cannot be longer than 50 bytes".into(),
            });
        }
